# ForecastapiSdk.TrafficForecastingApi
...
> TrafficForecastingResponse v2TrafficForecastingPost(trafficForecastingRequest)
...
let apiInstance = new ForecastapiSdk.TrafficForecastingApi();
...
apiInstance.v2TrafficForecastingPost(trafficForecastingRequest, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
